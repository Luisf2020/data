...
const MAX_TOKENS = ModelConfig[AgentModelName.gpt_4o].maxTokens * 0.7;
...
const generateSummary = async ({ text }: { text: string }) => {
  ...
  const response = await model.call({
    model: ModelConfig[AgentModelName.gpt_4o].name,
    messages: [
      {
        role: 'system',
        content:
    ...
    user: 'default-user', // O podemos omitirlo ya que ahora es opcional
  });
