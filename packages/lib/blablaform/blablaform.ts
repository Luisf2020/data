...
export class BlaBlaForm {
  ...
  constructor({
    schema,
    values = {},
    modelName = ModelConfig[AgentModelName.gpt_4o].name,
    messages = [],
...
`;
  ...
  static async blabla({
    ...
    if (isStreamEnabled) {
      ...
      const { answer, completion }: any = await model.call({
        ...callParams,
        handleStream: handleStream,
        user: 'default-user',
        stream: true,
        temperature: callParams.temperature || undefined,
        frequency_penalty: callParams.frequency_penalty || undefined,
        presence_penalty: callParams.presence_penalty || undefined,
        top_p: callParams.top_p || undefined,
        max_tokens: callParams.max_tokens || undefined,
      } as ModelCallParams);
      ...
      const { answer, completion }: any = await model.call({
        ...
        user: 'default-user',
        stream: handleLLMNewToken ? true : false,
      } as ModelCallParams);
