...
export const ChatRequest = ChatModelConfigSchema.extend({
  ...
  query: z.string(),
  queryArray: z.array(z.string()).optional().default([]),
    ...
    http_token: z.string().superRefine(async (arg, ctx) => {
      ...
      try {
        const { data: response } = await axios.post(
          `https://dashboard.chatsappai.com/api/tools/http-tool/validator`,
          {
...
export const ServiceProviderMetaSchema = ServiceProviderBaseSchema.extend({
  type: z.literal(ServiceProviderType.meta),
  config: z.object({
...
export type ServiceProviderMetaSchema = z.infer<
  typeof ServiceProviderMetaSchema
>;

export const ServiceProviderMercadolibreSchema = ServiceProviderBaseSchema.extend({
  type: z.literal(ServiceProviderType.mercadolibre),
  config: z.object({
...
export type ServiceProviderMercadolibreSchema = z.infer<
  typeof ServiceProviderMercadolibreSchema
>;

export const ServiceProviderSchema = z.discriminatedUnion('type', [
  ServiceProviderZendeskSchema,
  ServiceProviderWhatsappSchema,
  ServiceProviderMetaSchema,
  ServiceProviderMercadolibreSchema
]);
...
>;
export const AddServiceProviderMetaSchema = ServiceProviderMetaSchema.extend({
  agentId: z.string().cuid(),
});
export type AddServiceProviderMetaSchema = z.infer<typeof AddServiceProviderMetaSchema>;

// 
export const AddServiceProviderMercadolibreSchema = ServiceProviderMercadolibreSchema.extend({
  agentId: z.string().cuid(),
});

export type AddServiceProviderMercadolibreSchema = z.infer<typeof AddServiceProviderMercadolibreSchema>;
...
export type ServiceProviderWhatsapp = Extract<
  ...
  { type: 'whatsapp' }
>;
export type ServiceProviderMeta = Extract<
  ServiceProviderSchema,
  { type: 'meta' }
>;
export type ServiceProviderMercadolibre = Extract<
  ServiceProviderSchema,
  { type: 'mercadolibre' }
>;
...
export const AppToolSchema = ToolBaseSchema.extend({
...
export type AppToolSchema = Extract<ToolSchema, { type: 'app' }>;
...
export const CreateAgentSchema = z.object({
  ...
  useConversationalMode: z.boolean().optional().nullable(),
  useContextDataAgents: z.boolean().optional().nullable(),
  conversationalModePrompt: z.string().optional().nullable(),
  timeBetweenMessages: z.number().optional().nullable(),
