    ...
    const chatModel = traceable(
      async ({ messages }: { messages: { role: string; content: string }[] }) => {
        return chatv3({
          ...
          topK: this.topK,
          agentPortkey: `${this.agent.name} - ${this.agent.id}`,
          // Behaviors
          useLanguageDetection: !!this.agent.useLanguageDetection,
          restrictKnowledge: !!this.agent.restrictKnowledge,
          useContextDataAgents: !!this.agent.useContextDataAgents,
          useMarkdown: !!this.agent.useMarkdown,
        });
      },
      {
        name: `${this.agent.id}_${this.agent.name}_${props.conversationId}`,
        metadata: {
          ls_provider: "custom_provider",
          ls_model_name: this.agent.modelName,
          organizationId: this.agent.organizationId!,
          conversationId: props.conversationId,
          modelName: this.agent.modelName,
          filters: props.filters,
