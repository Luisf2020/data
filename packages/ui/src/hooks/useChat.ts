...
//const handleFeedback = new ChatModel("", "");
...
export const handleEvalAnswer = async (props: {
  ...
  try {
    // Enviar feedback a tu API local
...
export default function useChat({
  ...
  const getConversationQuery = useSWRInfinite(
    (pageIndex, previousPageData) => {
      ...
      return !disableFetchHistory
        ? `${API_URL ?? ''}/api/conversations/${state.conversationId}?cursor=${cursor || ''
        }`
        : null;
    },
    fetcher,
  ...
  const handleChatSubmit = useCallback(
    async ({
      ...
      if (!query?.trim() && files.length === 0 && !query?.trim() && attachmentsForAI.length === 0) {
        return;
      }
      ...
      if (files?.length > 0) {
        const filesUrls = await upload(
          files.map((each) => {
            const isAudioFile = each.type.startsWith('audio/');
            return {
              conversationId,
              agentId: otherProps.agentId!,
              case: 'chatUpload',
              fileName: isAudioFile
                ? `${each.name} -IGNORAR-NI-MENCIONARLO`
                : each.name,
              mimeType: each.type,
              file: each,
            };
