...
export default function CreateDatastoreModal(props: Props) {
  ...
  const steps = [
    {
      label: 'Elegir una fuente de datos',
          ...
          <Stepper activeStep={state.activeStep} orientation="vertical">
            {steps.map((step, index) => (
              <Step key={step.label}>
                <StepLabel
                  optional={
                    index === 3 ? <Typography>Último paso</Typography> : null
                  }
                >
                  {step.label}
                </StepLabel>
                <StepContent>
                  <div className="flex flex-col space-y-4">
                    ...
                    {index === 0 && (
                      <Alert color="primary">
                        ChatsappAI funciona mejor con datos no estructurados.
                        Mejora el soporte para datos tabulares (csv, hoja de
                        cálculo, etc...) pronto 😉
                      </Alert>
                    )}
