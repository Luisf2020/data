...
import AppToolModal from '../composio/modal/AppToolModal';
type Props = {
    ...
    <Stack gap={1}>
      {tools.length === 0 && (
        <Alert
          ...
          variant="soft"
        >
          Entrena a tu Agente con datos personalizados conectándolo a un Datastore a continuación.
        </Alert>
      )}
      ...
      <ToolCard
        id="app-tool"
        type={ToolType.app}
        name={agentToolConfig.app.title}
        description={agentToolConfig.app.description}
        mode="create"
        onCreate={appToolModal.open}
      />
      ...
      <appToolModal.component
        title={agentToolConfig.app.title}
        description={agentToolConfig.app.description}
        dialogProps={{
          sx: {
            maxWidth: 'md',
      ...
      >
        <AppToolModal
          ...
          onSubmit={(values: any) => {
            const existingToolIndex = tools.findIndex(tool =>
              tool.type === 'app' &&
              (tool.config?.actionId?.toLowerCase() === values.config.actionId?.toLowerCase() || tool.config?.actionId?.toLowerCase() === values.config.triggerId?.toLowerCase())
            );

            if (existingToolIndex !== -1) {
              const toolId = tools[existingToolIndex].id;

              if (toolId) {
                handleDeleteTool(toolId);
                tools.splice(existingToolIndex, 1);
              }
            } else {
              setValue('tools', [...tools, createTool(values)], {
                shouldDirty: true,
      ...
      >
        {state.currentToolIndex >= 0 && (
          <Stack gap={2}>
            ...
            >
              {isToolValidRef.current ? 'Actualizar' : 'Validar Configuración'}
            </Button>
          </Stack>
        )}
      </editApiToolForm.component>
