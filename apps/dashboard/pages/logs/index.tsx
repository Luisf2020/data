...
export default function LogsPage() {
  ...
  const updateStatusAllMutation = useSWRMutation<
    ...
    UpdateStatusAllConversationsSchema
  >(() => {
    return `/api/conversations/update-status${typeof window !== undefined ? window.location.search : ''
      }`;
  }, generateActionFetcher(HTTP_METHOD.POST));
  ...
  >(() => {
    return `/api/messages/mark-all-read${typeof window !== undefined ? window.location.search : ''
      }`;
  }, generateActionFetcher(HTTP_METHOD.POST));
    ...
    return (
      ...
      >
        <Stack justifyContent={'center'} alignItems={'center'} gap={1}>
          ...
          <Stack>
            <Typography level="body-md">No hay datos</Typography>
            <Typography level="body-sm">
              Todas las conversaciones con tus agentes serán visibles aquí
            </Typography>
          </Stack>
        </Stack>
      </Alert>
          ...
          <Dropdown>
            ...
            <Menu placement="bottom-start" size="sm">
              <MenuItem
                ...
                onClick={async () => {
                  const confirmed = window.confirm(
                    'Todos los mensajes que coinciden con los filtros serán marcados como leídos. ¿Estás seguro?'
                  );
              ...
              <MenuItem
                ...
                onClick={async () => {
                  const confirmed = window.confirm(
                    'Todas las conversaciones que coinciden con los filtros serán marcadas como resueltas. ¿Estás seguro?'
                  );
          ...
          >
            ...
            >
              <ChatBox
                ...
                channel={getConversationQuery?.data?.channel}
                        ...
                        {
                          {
                            ...
                            [ConversationStatus.UNRESOLVED]:
                              'Esta conversación todavía no ha sido resuelta.',
                            [ConversationStatus.HUMAN_REQUESTED]:
                              'Se ha solicitado asistencia humana.',
                          }[getConversationQuery?.data?.status]
                        }
                      </p>
                    </Alert>
