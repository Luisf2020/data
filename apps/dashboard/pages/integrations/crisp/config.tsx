...
export default function CrispConfig(props: { agent: Agent }) {
  ...
  return (
    <>
      <Head>
        <title>ChatsappAI- Automatización de LLMs sin código</title>
        <meta
          name="description"
          content="La mayoría del software de contabilidad es preciso, pero difícil de usar. Hacemos el intercambio opuesto, y esperamos que no te auditen."
        />
      </Head>
      {/* <Header /> */}
      <Box className="flex flex-col items-center justify-center w-screen h-screen p-4 overflow-y-auto bg-black">
        <Stack className="w-full max-w-sm mx-auto" gap={2}>
          {submitError && (
            <Alert color="danger">¡Ups! Clave API inválida</Alert>
          )}

          {showSuccessAlert && (
            <Alert color="success">
              ¡Configuración guardada! Ahora puedes cerrar esta ventana.
            </Alert>
          )}
          ...
          <Card variant="outlined">
            ...
            <form className="flex flex-col">
              <Stack spacing={2}>
                {!organization && (
                  <FormControl>
                    <FormLabel>Clave API de ChatsappAI</FormLabel>
                    <Alert variant="outlined" sx={{ mb: 2 }}>
                      <Stack>
                        Puedes encontrar tu Clave API en tus{' '}
                        <Link
                ...
                {!isPremium && isApiKeyValid && (
                  <Alert color="warning">
                    Esta es una característica premium. Por favor, actualiza tu
                    plan para usar la integración de Crisp.
                  </Alert>
                )}

                {isPremium && isApiKeyValid && (
                  <FormControl>
                    <FormLabel>Agente para conectar a Crisp</FormLabel>
                    <Select
